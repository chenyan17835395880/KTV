...
$(function () {
    ...
    $(window).on('hashchange', function () {
        ...
        if (location.hash == '#list') {
            $.ajax({
                url: '/KTV/index.php/musicmanager/show', //发送数据
                dataType: 'json',
    ...
    sub.on('click', function () {
        ...
        /*console.log(data)//FormData {}
                        // __proto__: FormData*/
        $.ajax({
            url: '/KTV/index.php/musicmanager/insert',
            type: 'post',
            data: data,
            success: function success(data) {
                if (data == "ok") {
    ...
    image.onchange = function () {
        ...
        [].concat(_toConsumableArray(this.files)).forEach(function (element, index) {
            ...
            reader.onload = function (e) {
                ...
                xml.open('post', '/KTV/index.php/musicmanager/upload', true);
    ...
    tbody.on('click', '.btn', function () {
        ...
        var that = this;
        $.ajax({
            url: '/KTV/index.php/musicmanager/delete',
            data: { id: id }, ///////////?????????????
            success: function success(data) {
                if (data == 'ok') {
    ...
    tbody.on('blur', 'input', function () {
        ...
        var id = $(this).parents('tr').attr('id');
        $.ajax({
            url: '/KTV/index.php/musicmanager/update',
    ...
    function rander(data) {
        ...
        $.each(data, function (index, value) {
            str += '\n                <tr id="' + value['fid'] + '">\n                    <td style="text-align: center" type="fid">' + value["fid"] + '</td>\n                    <td type="">\n                        <input type="ftitle" value="' + value["ftitle"] + '">\n                    </td>\n                    <td type="">\n                        <img src="' + value["images"] + '" alt="" width="90">    \n                    </td>\n                    <td style="text-align: center">\n                        <botton class="btn btn-info" style="background: #272924;border-color: #272924;">\u5220\u9664</botton>\n                    </td>\n                </tr>\n            ';
        });
