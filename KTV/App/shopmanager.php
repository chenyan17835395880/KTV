...
/*        var_dump( $data);*/
  ...
  values ('{$type}','{$sname}','{$hot}','{$price}','{$description}','{$srl}')");*/
        $this->mysql->query("insert into `shop` $strs");
//        echo ("insert into `shop` $strs");
    ...
    function delete(){
        $id=$_REQUEST['id'];
        $data=$this->mysql->query("delete from shop where sid=$id");
        if($this->mysql->affected_rows){
            echo 'ok';
        }else{
            echo 'error';
    ...
    function update(){
        ...
        $type=$_REQUEST['type'];
        $data=$this->mysql->query("update shop set $type='{$newval}' where sid=$id");
        if($this->mysql->affected_rows){
            echo 'ok';
        }else{
            echo 'error';
