...
class singermanager{
  ...
  values ('{$type}','{$sname}','{$hot}','{$price}','{$description}','{$srl}')");*/
        $this->mysql->query("insert into `singer` $strs");
//        echo ("insert into `shop` $strs");
    ...
    function delete(){
        $id=$_REQUEST['id'];
        $data=$this->mysql->query("delete from `singer` where sid=$id");
        if($this->mysql->affected_rows){
            echo 'ok';
        }else{
            echo 'error';
    ...
    function update(){
        ...
        $type=$_REQUEST['type'];
        $data=$this->mysql->query("update `singer` set $type='{$newval}' where sid=$id");
        if($this->mysql->affected_rows){
            echo 'ok';
        }else{
            echo 'error';
